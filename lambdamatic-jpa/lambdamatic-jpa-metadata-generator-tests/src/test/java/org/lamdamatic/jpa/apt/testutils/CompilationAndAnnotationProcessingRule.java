...
import org.lambdamatic.jpa.apt.EntityMetadataGenerator;
...
public class CompilationAndAnnotationProcessingRule implements MethodRule {
	...
	private void processAndCompile(final List<Class<?>> domainClasses) throws IOException {
		...
		final CompilationTask aptCompilationTask = compiler.getTask(null, fileManager, diagnosticListener, options, null,
				filesToCompile);
		aptCompilationTask.setProcessors(Arrays.asList(new EntityMetadataGenerator()));
		// operation
		final Boolean compiledWithoutErrorsWithAP = aptCompilationTask.call();
		assertThat(compiledWithoutErrors).as("Files compiled without errors").isTrue();
		assertThat(compiledWithoutErrorsWithAP).as("Files compiled without errors").isTrue();
	}
