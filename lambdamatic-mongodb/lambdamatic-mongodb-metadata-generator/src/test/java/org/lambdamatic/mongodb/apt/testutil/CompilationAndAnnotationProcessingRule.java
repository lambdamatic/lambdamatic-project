...
public class CompilationAndAnnotationProcessingRule implements MethodRule {
	...
	private final static File SRC_TEST_JAVA_FOLDER = toFile("src", "test", "java");
	/** The "target/generated-test-sources/" folder in which all test sources should be found. */
	private final static File TARGET_GENERATED_TEST_SOURCES_FOLDER = toFile("src", "test", "generated");
	/** The "target/test-classes/" folder in which all test sources should be found. */
	private final static File TARGET_TEST_CLASSES_FOLDER = toFile("target", "test-classes/");
	 ...
	 */
	private static File toFile(final String... pathFragments) {
		return Paths.get(System.getProperty("user.dir"), pathFragments).toFile();
	}
	...
	/**
	 * Clean the {@code TARGET_GENERATED_TEST_SOURCES_FOLDER folder (src/test/generated)} before running the test.
	 * 
	 * @throws IOException
	 */
