...
/**
 * A type (or parameterized) type to be used in the class templates. Provides the simple Java type name along with all
 * types to declare as import statements.
 * <p>
...
public class TemplateType extends TemplateElement {
	...
	private static final Function<TypeMirror, TemplateType> primitiveTypeToQueryType = variableType -> Builder
			.with(QueryField.class)
			.withTypeParameters(ElementUtils.getSimilarDeclaredType((PrimitiveType) variableType)).build();

	private static final Function<TypeMirror, TemplateType> primitiveTypeToProjectionType = variableType -> Builder
			.with(ProjectionField.class).build();

	private static final Function<TypeMirror, TemplateType> primitiveTypeToUpdateType = variableType -> Builder
			.with(ElementUtils.getSimilarDeclaredType((PrimitiveType) variableType)).build();

	private static final Function<DeclaredType, TemplateType> embeddedDocumentToQueryType = declaredType -> Builder
			.with(MetadataTemplateContext.elementToPackageName.apply(declaredType) + '.'
					+ MetadataTemplateContext.elementToQuerySimpleClassNameFunction.apply(declaredType))
			.build();

	private static final Function<DeclaredType, TemplateType> embeddedDocumentToProjectionType = declaredType -> Builder
			.with(MetadataTemplateContext.elementToPackageName.apply(declaredType) + '.'
					+ MetadataTemplateContext.elementToProjectionSimpleClassName.apply(declaredType))
			.build();

	private static final Function<DeclaredType, TemplateType> embeddedDocumentToUpdateType = declaredType -> Builder
			.with(MetadataTemplateContext.elementToPackageName.apply(declaredType) + '.'
					+ MetadataTemplateContext.elementToUpdateSimpleClassNameFunction.apply(declaredType))
			.build();

	private static final Function<TypeMirror, TemplateType> collectionToQueryType = collectionType -> Builder
			.with(QueryArray.class).withTypeParameters(getQueryMetadataFieldType(collectionType)).build();

	private static final Function<TypeMirror, TemplateType> collectionToProjectionType = collectionType -> Builder
			.with(ProjectionArray.class).withTypeParameters(getQueryMetadataFieldType(collectionType)).build();

	private static final Function<TypeMirror, TemplateType> collectionToUpdateType = collectionType -> Builder
			.with(UpdateArray.class).parameterTypes(collectionType.toString()).build();

	private static final Function<DeclaredType, TemplateType> mapToQueryType = mapType -> Builder.with(QueryMap.class)
	...
	private static final Function<DeclaredType, TemplateType> mapToProjectionType = mapType -> Builder
			.with(ProjectionMap.class)
			.withTypeParameters(Builder.with(mapType.getTypeArguments().get(0).toString()).build(),
					getQueryMetadataFieldType(mapType.getTypeArguments().get(1)))
			.build();

	private static final Function<DeclaredType, TemplateType> mapToUpdateType = mapType -> Builder.with(UpdateMap.class)
			.withTypeParameters(Builder.with(mapType.getTypeArguments().get(0).toString()).build(),
					getQueryMetadataFieldType(mapType.getTypeArguments().get(1)))
			.build();

	private static final Function<DeclaredType, TemplateType> declaredTypeToQueryType = declaredType -> {
	...
	private static final Function<DeclaredType, TemplateType> declaredTypeToProjectionType = declaredType -> Builder
			.with(ProjectionField.class).build();

	private static final Function<DeclaredType, TemplateType> declaredTypeToUpdateType = declaredType -> {
		switch (declaredType.toString()) {
		case "org.lambdamatic.mongodb.types.geospatial.Location":
			return Builder.with(LocationField.class).build();
		default:
	...
	public static TemplateType getProjectionMetadataFieldType(final TypeMirror variableType)
			throws MetadataGenerationException {
		return getMetadataFieldType(variableType, primitiveTypeToProjectionType, embeddedDocumentToProjectionType,
				collectionToProjectionType, mapToProjectionType, declaredTypeToProjectionType);
	}
	...
	public static TemplateType getUpdateMetadataFieldType(final TypeMirror variableType)
			throws MetadataGenerationException {
		return getMetadataFieldType(variableType, primitiveTypeToUpdateType, embeddedDocumentToUpdateType,
				collectionToUpdateType, mapToUpdateType, declaredTypeToUpdateType);
	}
	...
	private static TemplateType getMetadataFieldType(final TypeMirror variableType,
			...
			final Function<DeclaredType, TemplateType> declaredTypeToTemplateType) throws MetadataGenerationException {
	...
	static class Builder {
		...
		Builder withTypeParameters(final Class<?>... parameterTypes) {
			this.parameterTypes = Stream.of(parameterTypes)
					.map(c -> new TemplateType(c.getName(), Collections.emptyList())).collect(Collectors.toList());
			return this;
		}

		Builder parameterTypes(final String... parameterTypeNames) {
			this.parameterTypes = Stream.of(parameterTypeNames).map(t -> new TemplateType(t, Collections.emptyList()))
	 ...
	 *            the fully qualified name of the corresponding Java field type.
	 * @param parameterTypes
	 *            the Java parameter types.
	 */
	private TemplateType(final String fullyQualifiedName, final List<TemplateType> parameterTypes) {
		this.fullyQualifiedName = fullyQualifiedName;
	...
	public LinkedList<String> getJavaTypeParameters() {
		return javaTypeParameterNames;
	}

	/**
	 * @return all the Java types to declare in the imports (can be one or many if this {@link TemplateType} is for a
	 *         parameterized type)
	 */
	@Override
