...
public class TemplateAnnotation extends TemplateElement {
	...
	public static class Builder {
		...
		public TemplateAnnotation build() {
			return new TemplateAnnotation(type, attributes);
		}
	}
	 ...
	 *            the attributes to output in the template.
	 */
	private TemplateAnnotation(final TemplateType type, final Map<String, Object> attributes) {
		this.type = type;
		this.attributes = attributes;
	}

	/**
	 * Assumes that the annotation only requires its own type, ie, that the attribute values are _not_ nested annotations.
	 */
	@Override
	...
	public String toString() {
		...
		printBuilder.append('@').append(type.getSimpleName());
		final String attributesOutput = attributes.entrySet().stream()
				.map(entry -> {
