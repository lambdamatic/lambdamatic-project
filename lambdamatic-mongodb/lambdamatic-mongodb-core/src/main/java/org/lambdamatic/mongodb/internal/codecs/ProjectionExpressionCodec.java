 ...
 *
 */
public class ProjectionExpressionCodec
    extends BaseLambdaExpressionCodec<SerializableConsumer<ProjectionMetadata<?>>> {
  ...
  void encodeExpression(final LambdaExpression lambdaExpression, final BsonWriter writer,
    ...
    if (projectionType == null
        && methodInvocation.getParent().getExpressionType() == ExpressionType.LAMBDA_EXPRESSION
        && methodInvocation.getParent().getParent() == null) {
      throw new ConversionException("Invalid projection. See " + Projection.class.getName());
    }
    final List<Expression> arguments = methodInvocation.getArguments();
    if (arguments.size() != 1) {
      throw new ConversionException(
          "Invalid projection: missing fields or fields not wrapped in an array");
    }
  ...
  /**
   * Finds the {@link ProjectionType} associated with the given Java {@link Method}, depending on
   * whether it is annotated with {@link IncludeFields} ({@link ProjectionType#INCLUDE}) or with
   * {@link ExcludeFields} ( {@link ProjectionType#EXCLUDE}).
   * 
   * @param method the method to analyze
   * @return the corresponding {@link ProjectionType} or <code>null</code> if no relevant annotation
   *         was found on the given {@link Method}.
   */
  private static ProjectionType getProjectionType(final Method method) {
