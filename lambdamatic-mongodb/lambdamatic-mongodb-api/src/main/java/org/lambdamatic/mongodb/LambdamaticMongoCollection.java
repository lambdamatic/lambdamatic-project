 ...
 *
 */
public interface LambdamaticMongoCollection
      <DomainType, 
    ...
    extends FilterContext<DomainType, ProjectionType, UpdateType> {
   ...
   * @return the {@link FilterContext} element to carry on with the query using the given
  ...
  /**
   * @return the {@link FilterContext} element to carry on with the query <strong>matching all
   *         elements</strong>.
   */
  public FilterContext<DomainType, ProjectionType, UpdateType> all();

  /**
   * Adds (Inserts) the given {@code domainObjects} in the underlying MongoDB Collection. If no
   * {@code id} attribute (ie, annotated with {@link DocumentId}) was not set, a random value will
   * be provided.
   * 
   * @param domainObjects the domain objects to insert
   */
  public void add(@SuppressWarnings("unchecked") final DomainType... domainObjects);

  /**
   * "Upserts" the given {@code domainObject} in the Database, ie, of the domain object exists, it
   * is <strong>replaced</strong>, otherwise it is <strong>inserted</strong>.
   * 
  ...
  /**
   * Replaces (Updates) with the given {@code domainObject}, performing <strong>a full
   * replacement</strong> of the actual content identified by the {@link DocumentId} in the
   * associated MongoDB collection.
   * 
