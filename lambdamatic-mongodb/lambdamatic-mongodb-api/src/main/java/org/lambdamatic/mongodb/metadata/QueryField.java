...
public interface QueryField<T> {
	...
	/**
	 * The {@link QueryField#in(Object)} ({@code $in})  operator selects the documents where the value of a field equals any value in the specified array.
	 * If the current field holds an array, then the $in operator selects the documents whose field holds an array that contains at least one element that matches a value in the specified array (e.g. {@code value1}, {@code value2}, etc.)
