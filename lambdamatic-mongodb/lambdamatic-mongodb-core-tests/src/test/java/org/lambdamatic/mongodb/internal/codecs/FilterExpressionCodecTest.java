...
public class FilterExpressionCodecTest {
	...
	public static Object[][] data() {
		...
		data.match(foo -> !foo.primitiveBooleanField.equals(false), "{$not:{primitiveBooleanField: false}}");
		data.match(foo -> foo.primitiveByteField.equals(1), "{primitiveByteField: 1}");
		...
		data.match(foo -> foo.primitiveFloatField.notEquals(1.1f), "{primitiveFloatField: {$ne: 1.1}}}");
		...
		data.match(foo -> foo.primitiveFloatField.greaterThan(1.1f), "{primitiveFloatField: {$gt: 1.1}}}");
		...
		data.match(foo -> foo.primitiveFloatField.greaterOrEquals(1.1f), "{primitiveFloatField: {$gte: 1.1}}}");
		data.match(foo -> foo.primitiveCharField.greaterOrEquals('A'), "{primitiveCharField: {$gte: 'A'}}}");
		// $lt
		...
		data.match(foo -> foo.primitiveFloatField.lessOrEquals(1.1f), "{primitiveFloatField: {$lte: 1.1}}}");
		data.match(foo -> foo.primitiveCharField.lessOrEquals('A'), "{primitiveCharField: {$lte: 'A'}}}");
		// $in
		data.match(foo -> foo.stringField.in("John", "Jack"), "{stringField: { $in: ['John', 'Jack']}}");
		data.match(foo -> foo.stringField.in(namesList), "{stringField: { $in: ['John', 'Jack']}}");
		data.match(foo -> foo.enumFoo.in(EnumFoo.FOO, EnumFoo.BAR), "{enumFoo: { $in: ['FOO', 'BAR']}}");
		data.match(foo -> foo.enumFoo.in(enumList), "{enumFoo: { $in: ['FOO', 'BAR']}}");
		// FIXME: support this case for primitive types too

		// $nin
		data.match(foo -> foo.stringField.notIn("John", "Jack"), "{stringField: { $nin: ['John', 'Jack']}}");
		data.match(foo -> !foo.stringField.in("John", "Jack"), "{$not:{stringField: {$in: ['John', 'Jack']}}}");
		data.match(foo -> foo.stringField.notIn(namesList), "{stringField: { $nin: ['John', 'Jack']}}");
		data.match(foo -> !foo.stringField.in(namesList), "{$not:{stringField: { $in: ['John', 'Jack']}}}");
		data.match(foo -> foo.enumFoo.notIn(EnumFoo.FOO, EnumFoo.BAR), "{enumFoo: { $nin: ['FOO', 'BAR']}}");
		data.match(foo -> !foo.enumFoo.in(EnumFoo.FOO, EnumFoo.BAR), "{$not:{enumFoo: { $in: ['FOO', 'BAR']}}}");
		...
		// $not
		// FIXME: support this special operator with an annotated helper method ? Eg : Query.not(...), similar to
