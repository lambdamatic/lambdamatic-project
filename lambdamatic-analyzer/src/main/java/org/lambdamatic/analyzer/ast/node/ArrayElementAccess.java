...
/**
 * A somehow "fake" expression that corresponds to the access to a particular element in an array. In the Java DSL, this
 * is expressed by a method call such as {@code foo.bar(1)} where the {@code bar} method is annotated with
 * {@link ArrayElementAccess}.
 * 
 * <p>
 * Note: As opposed to a {@link FieldAccess}, there is no real underlying field in the Java code, so special treatment
 * is required during the encoding of any {@link Predicate} that might use this kind of element.
 * </p>
 */
public class ArrayElementAccess extends Expression {
	...
	public ArrayElementAccess(final MethodInvocation methodInvocation) {
		this(generateId(), (FieldAccess) methodInvocation.getSource(), methodInvocation.getArguments().get(0).toString(), methodInvocation.getReturnType(), false);
	}
	 ...
	 */
	public ArrayElementAccess(final int id, final FieldAccess sourceField, final String index, final Class<?> returnType, final boolean inverted) {
		super(id, inverted);
	...
	/**
	 * <strong>Note:</strong> The underlying Java type that is returned by this {@link ArrayElementAccess} expression.
	 */
	@Override
	...
	public String toString() {
		return sourceField.toString() + ".get(" + index + ")";
	}
}
