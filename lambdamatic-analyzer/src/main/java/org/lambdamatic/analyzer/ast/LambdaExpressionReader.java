...
public class LambdaExpressionReader {
	 ...
	 * @param implClass the fully qualified name of the implementation class
	 * @param implMethodName the name of the implementation method
	 * @param implMethodSignature the signature of the implementation method
	 * @param capturedArgs the captured arguments
	 * 
	...
	public Statement readBytecodeStatement(final SerializedLambdaInfo lambdaInfo) throws IOException {
		...
		final InputStream serializedLambdaStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(lambdaInfo.getImplClassName().replace('.', '/')
                + ".class");
		...
		insnCursor.next();
		return readStatementSubTree(insnCursor, new Stack<>(), lambdaInfo.getCapturedArgs(), localVariables);
	}
	...
	private Statement readStatementSubTree(final InsnCursor insnCursor, final Stack<Expression> expressionStack, 
		...
		while (insnCursor.hasCurrent()) {
			...
			case AbstractInsnNode.METHOD_INSN:
				...
				case Opcodes.INVOKESPECIAL:
					...
					if(methodInsnNode.name.equals("<init>")) {
						...
						final Method javaMethod = ReflectionUtils.findJavaMethod(sourceExpression.getJavaType(), methodInsnNode.name, parameterTypes);
				...
				case Opcodes.INVOKESTATIC:
					...
					try {
						...
						final Method javaMethod = ReflectionUtils.findJavaMethod(sourceClass, methodInsnNode.name, parameterTypes);
						final MethodInvocation invokedStaticMethod = new MethodInvocation(new ClassLiteral(sourceClass), javaMethod, args);
			...
			case AbstractInsnNode.INVOKE_DYNAMIC_INSN:
				...
				final SerializedLambdaInfo lambdaInfo = new SerializedLambdaInfo(handle.getOwner(), handle.getName(), handle.getDesc(), Collections.emptyList());
				final LambdaExpression lambdaExpression = LambdaExpressionAnalyzer.getInstance().analyzeExpression(lambdaInfo);
				expressionStack.add(lambdaExpression);
				//throw new AnalyzeException("Nested InvokeDynamic instruction is not supported yet. Should analyze " + handle.getOwner() + "." + handle.getName() + " (desc=" + handle.getDesc() + ")");
				break;
