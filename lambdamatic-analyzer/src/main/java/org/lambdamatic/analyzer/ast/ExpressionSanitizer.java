...
/**
 * Creates a new {@link Expression} where method calls such as {@link Long#longValue()}, etc. are
 * removed because they are not relevant in our case (they are just underlying conversion methods)
 * Finally, {@link CompoundExpression} with a boolean conditions (eg: {@link MethodInvocation} ==
 * {@link BooleanLiteral}) are simplified as well: the {@link CompoundExpression} is replaced with
...
public class ExpressionSanitizer extends ExpressionVisitor {
  ...
  /**
   * If the {@link CompoundExpression} with a boolean conditions (eg: {@link MethodInvocation} ==
   * {@link BooleanLiteral}), replace it with the meaningful operand (which may need to be
   * inverted).
   * 
  ...
  public boolean visitMethodInvocationExpression(final MethodInvocation methodInvocation) {
    if (Stream.of(new MethodMatcher(Boolean.class, "booleanValue"),
        new MethodMatcher(Byte.class, "byteValue"), new MethodMatcher(Short.class, "shortValue"),
        new MethodMatcher(Integer.class, "intValue"), new MethodMatcher(Long.class, "longValue"),
    ...
    } else if (new MethodMatcher(Boolean.class, "valueOf", boolean.class)
  ...
  public boolean visitFieldAccessExpression(final FieldAccess fieldAccess) {
    if (fieldAccess.getSource().getExpressionType() == ExpressionType.CLASS_LITERAL) {
      ...
      try {
        ...
        throw new AnalyzeException("Failed to execute method '" + fieldName + "' on class '"
            + sourceClass.toString() + "'", e);
      }
  ...
  static class MethodMatcher {
     ...
     * @return <code>true</code> if the given {@link MethodInvocation} matches on the source, method
     *         name and argument types, <code>false</code> otherwise.
     */
