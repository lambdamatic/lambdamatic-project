...
public class CapturedArgumentsEvaluatorTest {
	...
	public void shouldSubstituteCapturedArgumentInTwoMethodInvocationSourceExpressions() {
		...
		final CompoundExpression expression = new CompoundExpression(CompoundExpressionOperator.CONDITIONAL_OR, equalsMethodInvocation, equalsFieldAccess);
		// when
		...
		// then
		final CompoundExpression expectedResult = new CompoundExpression(CompoundExpressionOperator.CONDITIONAL_OR, new MethodInvocation(testPojo, Object_equals, 
	...
	public void shouldSubstituteCapturedArgumentRefInMethodInvocationArguments() {
		...
		final CompoundExpression actualExpr = new CompoundExpression(CompoundExpressionOperator.CONDITIONAL_OR, getStringValueEqualsBar, getPrimitiveIntValueEquals42);
		// when
		final CapturedArgumentsEvaluator expressionRewriter = new CapturedArgumentsEvaluator(capturedArguments);
		actualExpr.accept(expressionRewriter);
		// then
		final CompoundExpression expectedExpr = new CompoundExpression(CompoundExpressionOperator.CONDITIONAL_OR, 
